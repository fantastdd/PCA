...
import essential.Setting;
import essential.Constraint;


public class ExtendSimulator {
// matrix -- > graph. 
	...
	public ExtendSimulator(Setting setting)
	
	{
		this.setting = setting;
	}
	public ExtendSimulator()
	...
	private void extendToAModel(PCGraph graph)
	{
		
		for (Constraint constraint:graph.getAllConstraints())
		{
			int rel = graph.getEdges()[(Integer) constraint.getSource()][(Integer) constraint.getDestination()];
		//	   System.out.println("Before: "+BinaryRelations.translateToString(graph.getNodes()[(Integer) constraint.getSource()][(Integer) constraint.getDestination()]));	
			rel = ExtendStrategy.extend(rel);
			graph.getEdges()[(Integer) constraint.getSource()][(Integer) constraint.getDestination()] = rel;
			
		//    System.out.println("After:  "+BinaryRelations.translateToString(graph.getNodes()[(Integer) constraint.getSource()][(Integer) constraint.getDestination()]));	
	    }
		graph.getAllConstraints().clear();
	...
	//	graph1.printGraph();
		  for (int i = 0;i<100000;i++)
		  {
			  PCGraph graph = simulator.buildGraph(4, 3, 2);
				...
				PCA = simulator.Path_Consistency(graph);
			    ExtendSimulator simulator1 = new ExtendSimulator();
				CA = simulator1.Consistency(clone);
				if (PCA == CA)
					counter ++;
